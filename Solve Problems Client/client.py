...
try:		#Check for the specified valid port number
	...
	print "Invalid input for port error"
	sys.exit()
...
try:		#Check for specified and valid hostname
	...
	print "Invalid hostname for CCS server"
	sys.exit()
try:
		assert(sys.argv[2]=='-p')
	except AssertionError:
...
nuID=sys.argv[5]
if len(nuID)==9 and nuID.isdigit(): #This condition checks for a valid NUI 
	s=ssl.wrap_socket(s)
	...
	while True:
			...
			try:									#Checks to see if timeout has occurred
